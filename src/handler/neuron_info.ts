...
export async function handleNeuronInfo(req: withBotClient, res: Response) {
    ...
    try {
        const response = await axios.get<NeuronInfo>(`${NEURON_INFO_API_URL}/${neuronId}`, { 
        ...
        if (!neuron) {
            throw new Error('Neuron data unavailable');
        }
        ...
        res.status(200).json({ 
            ...
            neuron: {
                ...
                isGtc: neuron.is_gtc,
                isKnown: neuron.is_known,
                updatedAt: neuron.updated_at
            }
        });
    } catch (error) {
