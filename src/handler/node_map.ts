...
export async function handleNodeMap(req: withBotClient, res: Response) {
  ...
  try {
    ...
    if (locations.length === 0) {
      throw new Error('No boundary node data available');
    }
    ...
    // Save image to file
    const outputPath = path.join(__dirname, 'icp-nodes-map.png');
    fs.writeFileSync(outputPath, mapImage);

    // Create and send message with image
    const fileMessage = await client.createFileMessage(
      outputPath,
      fs.readFileSync(outputPath),
      'image/png',
      fs.statSync(outputPath).size
    );
    fileMessage.setCaption('🌐 **ICP Node Map**\n\nVisual representation of global node distribution.');
    await client.sendMessage(fileMessage);

    res.status(200).json({ 
      success: true, 
      nodeCount: locations.reduce((sum, loc) => sum + loc.total_nodes, 0),
