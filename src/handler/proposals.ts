...
export async function handleProposals(req: withBotClient, res: Response) {
    ...
    try {
        ...
        if (proposals.length === 0) {
            const noProposalsMessage = await client.createTextMessage('🗳️ No matching proposals found.');
            await client.sendMessage(noProposalsMessage);
        ...
        const formattedProposals = proposals.map((proposal, index) => {
            const { id, title, topic, status, latest_tally, deadline_timestamp_seconds, url } = proposal;
            ...
            if (deadline_timestamp_seconds) {
                ...
                votingEndInfo = `Voting Ends: ${votingEndDate.toISOString().split('T')[0]}`;
            }

            // Calculate yes percentage if tally available
            let voteInfo = '';
            if (latest_tally && latest_tally.total > 0) {
                const yesPercentage = Math.round((latest_tally.yes / latest_tally.total) * 100);
            ...
            if (status === 'OPEN') {
                ...
                if (status === 'EXECUTED' && proposal.executed_timestamp_seconds) {
                    const executedDate = new Date(proposal.executed_timestamp_seconds * 1000);
            ...
            return `${index + 1}. #${id}: ${title}\n   🏷️ ${topic.replace('TOPIC_', '')} | ${statusInfo}${linkInfo}`;
        });

        const message = `🗳️ **Latest Proposals** (${proposals.length} found)\n\n${formattedProposals.join('\n\n')}`;
        ...
        try {
            ...
            // Continue with the API response even if message sending fails
        }
        ...
        res.status(200).json({ 
            ...
            proposals: proposals.map(p => ({
                ...
                deadline: p.deadline_timestamp_seconds 
                    ? new Date(p.deadline_timestamp_seconds * 1000).toISOString() 
                    : undefined
            }))
        });
    } catch (error) {
